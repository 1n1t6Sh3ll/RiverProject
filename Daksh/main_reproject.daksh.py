...
# MODIS enrichment (Earth Engine). Set False to skip MODIS sampling entirely.
ENABLE_MODIS  = True
EE_PROJECT    = "noaa-river-ice"

# IGBP class names live in modis_enrich.IGBP_LOOKUP
...
def main():
    ...
    ee = modis_enrich.init_ee(EE_PROJECT) if ENABLE_MODIS else None
    ...
    for date_str, v_list, l_list in matched:
        ...
        # Step 5 — CSV (candidate pixels, same as valen/) + MODIS enrichment
        for vp in viirs_outs:
            for lp, lp_b10, lp_mtl in landsat_outs:
                generate_csv(vp, lp, date_str, date_out, lp_b10, lp_mtl, ee)
